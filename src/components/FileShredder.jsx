...
export default function FileShredder() {
  ...
  const runTool = async () => {
    ...
    try {
      if (window.api) {
        const res = await window.api.runSystemCommand('run-advanced-tool', ['shred']);
        if (res.success) {
          addNotification('Success', 'Secure File Shredder operation completed.', 'success');
        ...
        setTimeout(() => {
          setResult('Mock output for Secure File Shredder');
          setLoading(false);
        }, 1000);
        return;
      }
  ...
  return (
    <div className="h-full flex flex-col space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight">Secure File Shredder</h1>
          <p className="text-slate-400 mt-1 text-sm">Permanently delete files so they can never be recovered.</p>
        </div>
        <button 
          onClick={runTool}
          disabled={loading}
