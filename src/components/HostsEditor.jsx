  ...
  const runTool = async () => {
    ...
    try {
      if (window.api) {
        ...
        if (res.success) {
          addNotification('Success', 'Hosts File Ad-Blocker operation completed.', 'success');
        ...
        setTimeout(() => {
          setResult('Mock output for Hosts File Ad-Blocker');
          setLoading(false);
        }, 1000);
        return;
      }
  ...
  return (
    <div className="h-full flex flex-col space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight">Hosts File Ad-Blocker</h1>
          <p className="text-slate-400 mt-1 text-sm">Block tracking servers and malicious domains at the system level.</p>
        </div>
        <button 
          onClick={runTool}
          disabled={loading}
