...
export default function DuplicateFinder() {
  ...
  const runTool = async () => {
    ...
    try {
      if (window.api) {
        const res = await window.api.runSystemCommand('run-advanced-tool', ['find-duplicates']);
        if (res.success) {
          addNotification('Success', 'Deep Duplicate Finder operation completed.', 'success');
        ...
        setTimeout(() => {
          setResult('Mock output for Deep Duplicate Finder');
          setLoading(false);
        }, 1000);
        return;
      }
  ...
  return (
    <div className="h-full flex flex-col space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight">Deep Duplicate Finder</h1>
          <p className="text-slate-400 mt-1 text-sm">Find and remove exact byte-to-byte duplicate files.</p>
        </div>
        <button 
          onClick={runTool}
          disabled={loading}
