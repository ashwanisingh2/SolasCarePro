...
export default function CoreParking() {
  ...
  const applyTweak = async () => {
    ...
    try {
      if (window.api) {
        const res = await window.api.runSystemCommand('run-power-tweak', ['unpark-cores']);
        if (res.success) {
          addNotification('Success', 'CPU Core Parking applied successfully.', 'success');
        ...
        setTimeout(() => {
          setResult('Mock successful operation for CPU Core Parking');
          setLoading(false);
        }, 1000);
        return;
      }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-200">CPU Core Parking</h2>
          <p className="text-xs text-slate-400">Unpark all CPU cores to prevent micro-stuttering in gaming workloads.</p>
        </div>
        <button 
          onClick={applyTweak}
          disabled={loading}
