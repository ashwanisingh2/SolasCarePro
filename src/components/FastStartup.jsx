...
export default function FastStartup() {
  ...
  const applyTweak = async () => {
    ...
    try {
      if (window.api) {
        const res = await window.api.runSystemCommand('run-power-tweak', ['disable-hibernation']);
        if (res.success) {
          addNotification('Success', 'Fast Startup / Hibernation applied successfully.', 'success');
        ...
        setTimeout(() => {
          setResult('Mock successful operation for Fast Startup / Hibernation');
          setLoading(false);
        }, 1000);
        return;
      }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-200">Fast Startup / Hibernation</h2>
          <p className="text-xs text-slate-400">Disable hibernation and fast startup to save SSD wear and fix memory leaks.</p>
        </div>
        <button 
          onClick={applyTweak}
          disabled={loading}
