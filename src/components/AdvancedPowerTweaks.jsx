...
export default function AdvancedPowerTweaks() {
  ...
  const applyTweak = async () => {
    ...
    try {
      if (window.api) {
        const res = await window.api.runSystemCommand('run-power-tweak', ['advanced-tweaks']);
        if (res.success) {
          addNotification('Success', 'Advanced Power Tweaks applied successfully.', 'success');
        ...
        setTimeout(() => {
          setResult('Mock successful operation for Advanced Power Tweaks');
          setLoading(false);
        }, 1000);
        return;
      }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-200">Advanced Power Tweaks</h2>
          <p className="text-xs text-slate-400">Disable USB Selective Suspend and PCIe Link State Power Management for raw device throughput.</p>
        </div>
        <button 
          onClick={applyTweak}
          disabled={loading}
