  ...
  const runTool = async () => {
    ...
    try {
      if (window.api) {
        ...
        if (res.success) {
          addNotification('Success', 'Locked File Unlocker operation completed.', 'success');
        ...
        setTimeout(() => {
          setResult('Mock output for Locked File Unlocker');
          setLoading(false);
        }, 1000);
        return;
      }
  ...
  return (
    <div className="h-full flex flex-col space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight">Locked File Unlocker</h1>
          <p className="text-slate-400 mt-1 text-sm">Force kill background processes keeping your files locked.</p>
        </div>
        <button 
          onClick={runTool}
          disabled={loading}
