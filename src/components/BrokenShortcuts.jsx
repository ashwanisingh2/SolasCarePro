  ...
  const runTool = async () => {
    ...
    try {
      if (window.api) {
        ...
        if (res.success) {
          addNotification('Success', 'Broken Shortcut Cleaner operation completed.', 'success');
        ...
        setTimeout(() => {
          setResult('Mock output for Broken Shortcut Cleaner');
          setLoading(false);
        }, 1000);
        return;
      }
  ...
  return (
    <div className="h-full flex flex-col space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight">Broken Shortcut Cleaner</h1>
          <p className="text-slate-400 mt-1 text-sm">Scan and remove empty folders and dead shortcuts.</p>
        </div>
        <button 
          onClick={runTool}
          disabled={loading}
