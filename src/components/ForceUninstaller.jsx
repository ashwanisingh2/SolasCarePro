  ...
  const runTool = async () => {
    ...
    try {
      if (window.api) {
        ...
        if (res.success) {
          addNotification('Success', 'Deep Force Uninstaller operation completed.', 'success');
        ...
        setTimeout(() => {
          setResult('Mock output for Deep Force Uninstaller');
          setLoading(false);
        }, 1000);
        return;
      }
  ...
  return (
    <div className="h-full flex flex-col space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight">Deep Force Uninstaller</h1>
          <p className="text-slate-400 mt-1 text-sm">Completely remove applications and leftover registry entries.</p>
        </div>
        <button 
          onClick={runTool}
          disabled={loading}
